...
## Theoretical Framework: Attention-Aware Geometric ASP
This project implements **Attention-Aware Geometric Answer Set Programming (AAG‑ASP)**, a geometric extension of classical Answer Set Programming that enables *reasoning directly over tensor-valued evidence* produced by neural models.

### ✨ Core Idea
Traditional neuro-symbolic ASP frameworks (e.g., NeurASP, DeepProbLog) collapse neural representations into discrete facts or probabilities **before reasoning**.  
AAG‑ASP instead preserves the *full tensor structure* of neural embeddings and introduces:

1. **Geometric predicates**  
   Predicates whose interpretation is not boolean but a tensor in ℝⁿ (e.g., embeddings from transformers or CNNs).

2. **Annotated geometric atoms**  
   Syntax: `p(c):v` where `v` is a tensor generated by a language or vision model.

3. **Projection functions**  
   Mappings πₚ: ℝⁿ → {0,1} used to convert tensor information into standard ASP atoms **without losing underlying geometric structure**.

4. **Attention-aware interpretations**  
   An interpretation includes:
   - truth values for ASP atoms  
   - tensors + attention weights for geometric atoms  
   These attention weights express contextual influence between geometric atoms.

5. **Attention-aware fixed-point semantics**  
   A modified immediate consequence operator propagates tensors using rule‑specific tensor functions and attention aggregation—**while classical stable model semantics is preserved** for the symbolic layer.

6. **Two-level learning cycle**  
   Learning updates the neural parameters **between** reasoning episodes, ensuring:
   - ASP reasoning remains non-monotonic and semantically intact  
   - neural representations are refined using symbolic feedback  

### ✨ Why this is different
- **Stable-model semantics remains unchanged** for symbolic atoms.  
- **Neural evidence becomes first‑class semantic objects**, not just inputs to classifiers.  
- **Contextual relations between tensors** (via attention) influence projected ASP atoms.  
- **Learning does not interfere with ASP semantics**—it occurs only after answer set computation.

This framework enables ASP programs to ground their conclusions in structured neural evidence, rather than flattened symbolic labels.
...
-   Demonstrates separation between perception and reasoning
-   Maintains formal stable model semantics
-   Supports cross-domain deployment (text & vision)
-   Enables meta-level learning without altering reduct semantics

------------------------------------------------------------------------

## Reproducibility Notes

-   All ASP programs are located in `/asp/`
-   Neural perception modules are modular and replaceable
-   Environment file includes exact dependencies
-   Code is deterministic for fixed random seeds
